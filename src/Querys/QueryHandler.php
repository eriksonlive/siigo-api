    ...
    public function getInvoice($id_factura)
    {
        $sql = "
            SELECT      
                ...
                CASE 
                WHEN cu.idtype=51 and cont_ac_correo is not null and cont_ac_correo!=''  THEN coalesce(cont_ac_correo,cu.email) 
            ...
            ar.notes as notas, 
                ...
                ac.transdate as fecha_pago	
                    ...
                    INNER JOIN ( --encuentra unico reg por resolu en fe_emisor.. sino existe configurac para la sede, trae el reg con dep_id=-1	            
                        SELECT *,case when department_id='-1' then (SELECT department_id FROM ar WHERE id=:id) else department_id end  department_esperado 
                        FROM fe_emisor  
                        WHERE  ( department_id in(SELECT department_id FROM ar WHERE id=:id) or department_id='-1')  
                ...
                INNER JOIN chart ch on ac.chart_id=ch.id  and  ch.link  ='AR_paid:AP_paid'
